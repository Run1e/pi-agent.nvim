...
export type EventListener<K extends keyof NvimEvents> = (dispatcher: Dispatcher, data: NvimEvents[K]) => void;

export const listenRegisterEventInterest: EventListener<"register_event_interest"> = (dispatcher, data) => {
	const ed = dispatcher.eventData;

	// register listener if not already
	if (!ed.registeredListeners.has(data.event_name)) {
		const castedOn = dispatcher.pi.on as (event: string, handler: (event: unknown) => unknown) => void;
		...
		castedOn(data.event_name, async (event) => {
			...
			if (ed.blockingListeners.get(data.event_name) ?? false) {
				const p = dispatcher.waitForEvent("pi_event_response", (event) => event.correlation_id === correlationId);
				...
				try {
					...
					dispatcher.ctx.ui.notify(`[pi-agent] Timed out waiting for blocking result for event '${data.event_name}'`);
					return;
				}
